...
export default function Hierarchy() {
	...
	const loadTeams = async (currentPage = 1) => {
		try {
			setLoading(true);
			const res = await getTeams({ page: currentPage });
	...
	const handleCreate = async (e) => {
		...
		try {
			...
			setError(
				err.response?.data?.error || "Ошибка при создании команды",
			);
		}
	};
	...
	if (loading)
		return <div className="hierarchy__status">Загрузка иерархии...</div>;
	if (error)
		return (
			<div className="hierarchy__status hierarchy__status--error">
				{error}
			</div>
		);
		...
		<section className="hierarchy container">
			...
			{isCreating && (
				<div className="hierarchy__create-wrapper">
					<form
						className="hierarchy__form form-block"
						onSubmit={handleCreate}
					>
						<div className="hierarchy__field form-block__field">
							<label className="hierarchy__label form-block__label">
								Название команды *
							</label>
							<input
								type="text"
						...
						<div className="hierarchy__field form-block__field">
							...
							<input
								...
								required
							/>
							<small className="hierarchy__help">
								Уровни идут по убыванию (1 — высший)
							</small>
						</div>

						<button
							type="submit"
							className="hierarchy__submit form-block__submit"
						>
							Создать команду
						</button>
					</form>
					...
					{Array.from(new Set(teams.map((t) => t.level)))
						...
						.map((level) => (
							<div key={level} className="hierarchy__level">
								<div className="hierarchy__level-badge">
									Уровень {level}
								</div>
								<div className="hierarchy__level-grid">
									{teams
										...
										.map((team) => (
											...
											>
												<h4 className="hierarchy__team-name">
													{team.name}
												</h4>
												<div className="hierarchy__team-meta">
													{team.league && (
														<span className="hierarchy__team-league">
															{team.league}
														</span>
													)}
													<span className="hierarchy__team-season">
														{team.season}
													</span>
												</div>
											</article>
