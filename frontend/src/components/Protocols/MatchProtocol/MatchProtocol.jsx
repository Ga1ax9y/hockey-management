...
export default function MatchProtocol() {
			...
			try {
				...
				existingStats.forEach((stat) => {
					allParticipantsMap.set(stat.playerId, {
						...
						player: stat.player,
						isHistorical:
							stat.player.currentTeamId !== data.myTeamId,
					});
				});
	...
	const saveRow = async (playerData) => {
		...
		try {
			...
			setPlayers((prev) =>
				prev.map((p) =>
					p.playerId === playerData.playerId
						? { ...p, ...savedData }
						: p,
				),
			);
					...
					<thead>
						<tr className="protocol-table__row">
							...
							{[
								...
								<th key={h} className="protocol-table__th">
									{h}
								</th>
							))}
							<th className="protocol-table__th"></th>
								...
								>
									...
									<td className="protocol-table__td protocol-table__td--status">
										{savingRows.has(p.playerId) ? (
											<span className="protocol-table__sync-icon">
												...
											</span>
										) : (
											<span className="protocol-table__check-icon">
												✓
											</span>
										)}
									</td>
