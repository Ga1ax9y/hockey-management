...
export default function Header() {
	...
	const { isAdmin, isCoach, isManager, isAnalyst } = useRole();
		...
		<header className="header">
			...
			<nav className="header__nav-desktop">
				<ul>
					...
					{(isAnalyst || isAdmin) && (
						<li>
							<Link to="/management/analytics">Аналитика</Link>
						</li>
					)}
					{isAuthenticated && (
						<>
			...
			<nav className={`header__nav-mobile ${isMenuOpen ? "open" : ""}`}>
				<ul>
					...
					{(isManager || isAdmin) && (
						<li>
							<Link to="/management/teams" onClick={toggleMenu}>
								Иерархия
							</Link>
						</li>
					)}
					...
					{isAuthenticated && (
						<>
							...
							<li>
								<button
									onClick={handleLogout}
									className="header__logout-button mobile"
								>
									Выйти
								</button>
							</li>
